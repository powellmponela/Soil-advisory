...
# Paths
DATA_DIR = r"d:\dss\SOIL ADVISORY\Data"
OUTPUT_DIR = r"d:\dss\SOIL ADVISORY\outputs"
...
def main():
    ...
    # Predict for NSAF data
    print("\n--- Estimating for NSAF Data Points ---")
    model_df['predicted_AE_N'] = rf.predict(model_df[features])
    nsaf_ins, nsaf_ips, nsaf_iks = quefts_proxy_ins(model_df)
    model_df['INS_kg_ha'] = nsaf_ins
    model_df['base_yield_kg_ha'] = model_df['INS_kg_ha'] * 40
    model_df['yield_gap_kg_ha'] = np.maximum(0, TARGET_YIELD_KG_HA - model_df['base_yield_kg_ha'])
    model_df['N_demand_kg_ha'] = (model_df['yield_gap_kg_ha'] / model_df['predicted_AE_N']).clip(upper=300)
    
    nsaf_out_file = os.path.join(OUTPUT_DIR, "nsaf_advisory_results.csv")
    model_df.to_csv(nsaf_out_file, index=False)
    print(f"NSAF advisory map generated: {nsaf_out_file}")
