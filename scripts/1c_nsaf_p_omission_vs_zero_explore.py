...
roles=["soil_background","p_omission"]
...
labels={
    ...
    "p_omission":"P omission (−P): N120–P0–K40",
}
...
for j,year in enumerate(YEARS):
    ...
    resp=paired_site_response(q,"p_omission","soil_background")
    if not resp.empty:
        resp["year"]=year
        all_resp.append(resp)

    axm=fig.add_subplot(gs[1,j])
    last=response_map_on_ax(axm,resp,boundary,fixed_districts=fixed_districts,title=f"{year}: Yield response to N+K")
...
if last is not None:
    ...
    cb.set_label("Yield response to N+K (t ha$^{-1}$)")

fig.suptitle("Yield response to N+K: omission treatment compared with unfertilized control",fontsize=15,fontweight="semibold",y=.98)
fig.subplots_adjust(bottom=.13,top=.92)
save(fig,"1c_nsaf_p_omission_vs_zero_explore.png")

if all_resp:
    pd.concat(all_resp,ignore_index=True).to_csv(TABLES/"1c_nsaf_p_omission_vs_zero_explore_site_response.csv",index=False)
